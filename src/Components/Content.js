...
function Content() {
  ...
  const getallDocumentModule = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8081/api/document-module/getdocumentbymoduleid/${id}`
      );
  ...
  const getallVideoModule = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8081/api/video-module/getvideobymoduleid/${id}`
      );
      if (response.status === 200) {
  ...
  const getallImageModule = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8081/api/image-module/getimagebymoduleid/${id}`
      );
      if (response.status === 200) {
          ...
          <div className="col-md-6">
            <div className="row border p-2" style={{ marginRight: "5px" }}>
              ...
              >
                ...
                <a
                  href={`http://localhost:8081/document-module/${data.document}`}
                  target="_blank"
                >
                  {data.document}
                </a>
        ...
        {allimagesmodule.map((data) => (
          <div className="col-md-6">
            <div className="row border p-2" style={{ marginRight: "5px" }}>
              <div className="col-md-2">
                <img
                  src={`http://localhost:8081/image/${data.image}`}
                  alt="loading..."
                  width="80"
                  height="60"
                />
